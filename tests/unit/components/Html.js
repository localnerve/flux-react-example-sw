...
describe('html component', function () {
  ...
  it('should render a standard app manifest', function () {
    var links = testUtils.scryRenderedDOMComponentsWithTag(htmlComponent, 'link');
  ...
  it('should render css meta holders for external stylesheet loads', function () {
    var metas = testUtils.scryRenderedDOMComponentsWithTag(htmlComponent, 'meta');
    var cssMetas = metas.filter(function (meta) {
      return meta.getAttribute('itemprop') === 'stylesheet';
    });

    expect(cssMetas.length).to.equal(1);
    expect(cssMetas[0].getAttribute('content')).to.equal(testProps.otherStyles[0]);
  });
